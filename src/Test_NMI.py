...
# Restore model with model structure

save_model.restore(sess, model_path)

prediction = tf.argmax(our_model, 1)
corrected_prediction = tf.equal(prediction, tf.argmax(_y, 1))
accuracy = tf.reduce_mean(tf.cast(corrected_prediction, tf.float32))

# Print accuracy
for i in range(10, 50):
    predict = sess.run(prediction, feed_dict={x: X[i].reshape(1, 60)})
    print("Expected Value : ", np.argmax(Y, 1)[i], " Predicted value : ", predict)
